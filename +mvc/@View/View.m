classdef View < handle
    ...
    properties (Constant)
        ...
        hUITableModelsClass     = containers.Map(["Name", "Type", "Data", "Accuracy_Training", "Accuracy_Test"], ...
    ...
    methods 
        ...
        function set.hUITableModels(obj, t)
            assert(isa(t, "table"))
            assert(all(arrayfun(@(x) isMatchingClassTable(x), string(t.Properties.VariableNames))))
            function bool = isMatchingClassTable(columnName)
                bool = string(obj.hUITableModelsClass(columnName)) == string(class(t.(columnName)));
            end
        end
